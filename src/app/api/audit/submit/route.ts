...
export async function POST(request: NextRequest) {
  try {
    ...
    const ipCountry = request.headers.get('cf-ipcountry') || '' // Cloudflare header
    ...
    try {
      ...
      // Update session status to reflect failure
      await supabase
        .from('audit_sessions')
        .update({ status: 'error' })
        .eq('id', session.id)
    ...
    console.error('Unexpected error in /api/audit/submit:', err)
    return NextResponse.json(
